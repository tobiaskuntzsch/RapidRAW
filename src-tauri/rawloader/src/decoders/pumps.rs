...
impl<'a> BitPump for BitPumpLSB<'a> {
    ...
    fn peek_bits(&mut self, num: u32) -> u32 {
        while self.nbits < num {
            let inbits: u64 = if self.pos < self.buffer.len() - 3 {
                LEu32(self.buffer, self.pos) as u64
            } else {
                let mut a = [0u8; 4];
                let end = self.buffer.len() - self.pos;
                if end > 0 {
                    a[..end].copy_from_slice(&self.buffer[self.pos..self.pos+end]);
                }
                u32::from_le_bytes(a) as u64
            };
            self.bits |= inbits << self.nbits;
            self.pos += 4;
            self.nbits += 32;
        }
        (self.bits & ((1u64 << num) - 1)) as u32
    }
...
impl<'a> BitPump for BitPumpMSB<'a> {
    ...
    fn peek_bits(&mut self, num: u32) -> u32 {
        while self.nbits < num {
             let inbits: u64 = if self.pos < self.buffer.len() - 3 {
                BEu32(self.buffer, self.pos) as u64
            } else {
                let mut a = [0u8; 4];
                let end = self.buffer.len() - self.pos;
                if end > 0 {
    ...
    fn consume_bits(&mut self, num: u32) {
        self.nbits -= num;
        self.bits &= if self.nbits == 0 { 0 } else { u64::MAX >> (64 - self.nbits) };
    }
}
