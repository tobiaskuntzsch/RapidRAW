...
impl<'a> LjpegDecompressor<'a> {
  ...
  pub fn decode(&self, out: &mut [u16], x: usize, stripwidth: usize, width: usize, height: usize, dummy: bool) -> Result<(),String> {
    ...
    if self.predictor == 6 && self.sof.cps == 1 && self.sof.width == width * 2 && self.sof.height * 2 == height {
        return decode_dji_interleaved(self, out, x, stripwidth, width, height);
    }

    if self.sof.components.len() > 0 && self.sof.components[0].super_h == 2 && self.sof.components[0].super_v == 2 {
      return decode_ljpeg_420(self, out, width, height)
    } else if self.sof.components.len() > 0 && self.sof.components[0].super_h == 2 && self.sof.components[0].super_v == 1 {
    ...
    match self.predictor {
      1 => {
        match self.sof.cps {
          1 => decode_ljpeg_1component(self, out, x, stripwidth, width, height),
      ...
      6 => {
          if self.sof.cps == 1 {
              decode_ljpeg_predictor6_flat(self, out, width, height)
          } else {
              Err(format!("ljpeg: predictor 6 with {} components not supported", self.sof.cps).to_string())
          }
      }
