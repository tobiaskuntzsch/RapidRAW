...
import { Images, CheckCircle, XCircle, Loader2, Save } from 'lucide-react';
import Button from '../ui/Button';

export default function PanoramaModal({ isOpen, onClose, progressMessage, finalImageBase64, error, onSave, onOpenFile }) {
    ...
    if (error) {
      return (
        <>
          ...
          <p className="text-sm text-text-secondary text-center p-2 rounded-md max-h-40 overflow-y-auto">{String(error)}</p>
        </>
      );
    }
  ...
  const renderButtons = () => {
    ...
    if (savedPath) {
      return (
        <>
          <button onClick={handleClose} className="px-4 py-2 rounded-md text-text-secondary hover:bg-card-active transition-colors">
            Close
          </button>
  ...
  return (
    ...
    >
      <div
        className={`bg-surface rounded-lg shadow-xl p-6 w-full max-w-xl transform transition-all duration-300 ease-out ${show ? 'scale-100 opacity-100 translate-y-0' : 'scale-95 opacity-0 -translate-y-4'}`}
