  ...
  pub fn decode_compressed(&self, raw: &TiffIFD, width: usize, height: usize, cpp: usize, dummy: bool) -> Result<Vec<u16>,String> {
    if let Some(offsets) = raw.find_entry(Tag::StripOffsets) {
      ...
      Ok(decode_threaded_multiline(width, height, tlength, dummy, &(|strip: &mut [u16], row| {
        let tile_row = row / tlength;
        for col in 0..coltiles {
          let offset = offsets.get_usize(tile_row*coltiles+col);
          let src = &self.buffer[offset..];
          
          let decompressor = match LjpegDecompressor::new(src) {
              Ok(d) => d,
              Err(e) => {
                  eprintln!("Error creating LJPEG decompressor for tile at row {}, col {}: {}", tile_row, col, e);
                  continue;
              }
          };

          let bwidth = cmp::min(width, (col+1)*twidth) - col*twidth;
          let blength = cmp::min(height, (tile_row+1)*tlength) - tile_row*tlength;
          
          if let Err(e) = decompressor.decode(strip, col*twidth, width, bwidth, blength, dummy) {
              eprintln!("Error decoding LJPEG tile at row {}, col {}: {}", tile_row, col, e);
          }
        }
      })))
    } else {
