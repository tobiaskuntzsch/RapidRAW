...
const LumaWaveformDisplay = ({ data, width, height, maxVal, color }) => {
  ...
  useEffect(() => {
    ...
    const scale = 255 / Math.log(1 + maxVal);
...
export default function Waveform({ waveformData, onClose }) {
  const [displayMode, setDisplayMode] = useState('luma');
  ...
  return (
    <Draggable handle=".handle" bounds="parent">
      <div className="absolute top-4 left-4 w-[280px] bg-bg-secondary/80 backdrop-blur-sm rounded-lg shadow-lg z-50 text-text-secondary border border-surface/40">
